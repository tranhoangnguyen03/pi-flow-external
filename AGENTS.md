...
- Valid `Agent` profiles must come from `~/.pi/agent/subagents/*.md` and set `backend: claude` or `backend: codex`.
...
- Prefer backend-qualified profile names: `claude-*` and `codex-*`.
...
Use workflows when the user asks for explicit fan-out or multi-agent orchestration across Claude/Codex lanes. Do not use workflow for native Pi subagents; use the native subagent system instead.
