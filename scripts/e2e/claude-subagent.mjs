#!/usr/bin/env node
// Real end-to-end smoke for Claude Code-backed subagents.
//
...
// tool. The selected profile uses `backend: claude`, so a successful run proves
// the root pi agent launched a real Claude Code child through the extension.
//
// Usage:
...
async function main() {
  ...
  const tokenValue = `${options.claudeModel}-${options.claudeThinking}`;
  const expected = `CLAUDE_SUBAGENT_OK:${tokenValue}`;
  ...
  try {
    ...
    assert(transcriptText.includes(expected), `expected Claude subagent token not found: ${expected}`);
    assert(gitStatus.status === 0, `git status failed: ${gitStatus.stderr}`);
    assert(gitStatus.stdout.trim() === "", `fixture was modified:\n${gitStatus.stdout}`);

    console.log("PASS claude subagent E2E");
...
main().catch((error) => {
  console.error(`FAIL claude subagent E2E: ${error instanceof Error ? error.stack || error.message : String(error)}`);
  process.exitCode = 1;
});
