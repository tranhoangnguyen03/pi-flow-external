...
| Post-run accounting | **Full** — `total_cost_usd` in result event | **Partial** — tokens reported at `turn.completed`; cost is `null`, rendered as unknown, never `$0` | **Partial** — tokens in terminal result; cost unknown |
