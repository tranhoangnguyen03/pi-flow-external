...
A runtime release runs all five direct receipts: `agy`, `claude`, `codex`, `grok`, and `muse`. During development, also run the backend you changed:
...
When workflow runtime, scheduling, supervision, or tool integration changes, run the affected backend. Before a runtime release, run all five CLI workflow receipts and one named Pi harness workflow receipt:
...
1. Run `npm run check`.
2. Run all five direct backend receipts (`agy`, `claude`, `codex`, `grok`, `muse`). All five are mandatory.
3. Run all five supervised workflow receipts, plus one named Pi harness direct receipt and its workflow receipt when a real harness is registered.
