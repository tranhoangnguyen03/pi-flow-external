...
Provide exactly one workflow source: name, scriptPath, or script. Inline scripts start with a literal export const meta = { apiVersion: 1, name, description }, call agent() at least once, await every started call, and return JSON-serializable data. Project .pi/workflows are visible only when the project is trusted.
...
Background and supervision: set background:true on Agent or workflow to return a stable run ID while session-owned work continues. external_runs actions are list (optional workflowRunId/cursor/limit), inspect (runId, view summary|output|diagnostics, optional cursor/limitBytes), wait (runIds, mode any|all), and cancel (runId, optional reason). Wait returns only terminal outcomes and never cancels pending work; cancelling a workflow stops its active children, while cancelling one child is a catchable workflow error.
