...
1. A role profile for an existing external CLI harness (claude, codex, agy, or grok).
...
For branch 1 or 3: ask one question at a time, only when the answer is not already known. Collect enough information to write a focused profile: its intended work, boundaries (especially read-only versus file modification), useful output, validation expectations, and stop/escalation rules. For branch 1, recommend a backend (claude, codex, agy, or grok) and explain briefly; for branch 3, confirm which already-registered pi-* harness this role targets. Suggest a lowercase <harness>-<role> profile name; the suffix becomes the role callers use. Ask about model and thinking only when I want to pin them for branch 1/3 profiles; otherwise omit them — for branch 3, model/thinking are inherited from the named harness and must not be overridden to a different value. When ready, summarize once and call ${PROFILE_TOOL_NAME}.
...
const profileParameters = Type.Object({
  ...
  backend: Type.String({ description: "External CLI backend (claude, codex, agy, grok) or a registered named pi-* harness." }),
...
/** Is `backend` one of claude/codex/agy/grok (a genuine external CLI selector)? */
...
export function compileProfile(profile: SubagentProfile): string {
  ...
  if (!isExternalCli && !isPiHarnessProfile) {
    throw new Error("Profile backend must be claude, codex, agy, grok, or a registered pi-* harness name.");
  }
          ...
          smokeTest: async (reconciledProfile) => {
            ...
            try {
              ...
              const result = await spawnSubagent({
                ...
                // object, unlike claude/codex/agy/grok which resolve their own model
