...
Install and authenticate OpenCode separately (`opencode auth login`); use `opencode models` for available `provider/model` IDs. The adapter targets OpenCode **1.18.32** and runs `opencode run --format json`, passing the prompt over stdin. An omitted model uses OpenCode's configured default. For an exact override:
...
Choose an ID available in your own installation. Do not pin `thinking`: OpenCode variants are model-specific and unknown variants are silently ignored upstream, so this adapter rejects explicit thinking pins and does not forward inherited Pi thinking. Structured workflow `schema` is unsupported and rejected; plain text results remain available.

`danger` uses `--auto`; explicit OpenCode deny rules still apply. `readonly` and `edit` inject a private, deny-by-default agent: read/grep/glob, plus edit/write/patch at `edit`. Shell, task delegation, and other tools are denied in those restricted tiers. These are **native tool permission rules, not an OS sandbox**; plugins and MCP servers still load. Restricted tiers refuse an existing `OPENCODE_CONFIG_CONTENT` rather than overwrite it.

Completion requires zero exit, no error event, and a final root-session step with reason `stop` and nonempty text—not an intermediate `tool-calls` step. Resume uses `--session`. Root-step usage/cost is reported natively; total cost becomes unknown after a native `task` because child usage is not streamed. No budget cap is enforced. OpenCode may retry provider requests internally; this extension does not retry it. Free OpenCode models may reject restricted tool sets; use an authenticated provider to validate restricted tiers.
